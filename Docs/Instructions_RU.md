...
- **Скачать `RHI_API_Tools_Launcher.exe`** можно по ссылке:

**[RHI_API_Tools_Launcher.exe](https://github.com/GH-GAMES/RHI_API_Tools/blob/main/Launcher/RHI_API_Tools_Launcher.exe)**

- В папке `Resources` находится пресет текстового файла с параметром запуска:
...
> - **Имя файла должно быть строго `launch_parameter.txt`**, с расширением `.txt`. Если имя или расширение будет другим, плагин не сможет его использовать — в этом случае при следующем запуске будет создан новый файл с корректным именем.  
...
Вы можете заранее задать нужный параметр, либо создав файл `launch_parameter.txt` вручную, либо скопировав его из папки `Resources`.
